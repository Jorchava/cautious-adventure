...
### ParticleContainer API change in v8 (confirmed)

// ParticleContainer.addChild() was removed in v8.
// Use addParticle() with the new Particle type, OR use a regular Container
// for small particle counts (≤200). Container is simpler and works identically
// for burst effects where max performance isn't critical.
