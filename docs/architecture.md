...
> Portfolio slot game — frontend showcase.
...
**Neon Reels** is a browser-based 5-reel, 3-row video slot machine built to demonstrate level frontend engineering. The game features a neon/cyberpunk aesthetic rendered entirely in PixiJS v8, with state management via Pinia, a typed finite state machine, configuration-driven game logic, REST-backed win history, and full TDD coverage on the game engine.
